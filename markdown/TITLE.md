...
SLG46826は１チップあたり、NVM/EEPROMで１つずつ、SRAM（レジスタ）で２つの合計４スレーブアドレスを専有します。
つまり１つのI2Cバスに４個まで繋げられます（[@tbl:slave-address]）。

ユーザが使うのはレジスタを直接操作するアドレスのみです。他のアドレスへのアクセスは（自分が何をしているのか
理解していないのであれば）避けてください。今回の設計では８ビット中MSB２ビットを選択できるようにしてあります。
...
バス規格で予約済のアドレスと衝突しないように、A4ビットは1に固定されています。
...
基板の設計がシンプルなためにちょっと面倒なことになっていますが、ソフトで吸収できました。
サンプルコード（[@lst:sample-sourcecode]）の９８行目付近を確認してください。
...
以下にPythonサンプルコードを示します。ラズパイ４で動作確認済です。
I2Cアクセスには[SMBus2](https://pypi.org/project/smbus2/)ライブラリを使っています。
