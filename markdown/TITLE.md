...
このビット6/7が`1x`から`00`または`01`に変化するときにDFFのクロックが生成されます。
同じポートに連続して書き込みたいときは一旦ビット7/6に`1x`を書き込んでおく必要があります。
つまり、以下のような手順で書き込みます：
...
3. `8'b11XX_XXXX` をレジスタ`0x7A`に書き込む。 `XX_XXXX` は影響を与えない。
   * `| S | AAAA_AAAW | 0111_1010 (0x7A) | 11XX_XXXX | P |`

ちなみに、リピートスタートを使えばこれらの書き込みを一度に行えるはずですが、筆者の環境ではうまくいきませんでした。
...
一方、この本の設計ではI2Cアドレスを可変にするために単一電源レール仕様になっています。
