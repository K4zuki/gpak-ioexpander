...
GreenPAKをI2C汎用出力（GPO）エキスパンダにして、2行（row）・6列（column）のリレーマトリクスが載った
ラズパイの拡張ボード（HAT）を設計しました。
...
1. `8'bPPDD_DDDD` をレジスタ`0x7A`に書き込む。ここで`PP`は`00`または`01`で、`DD_DDDD`は書き込みたいデータ
1. `8'b11XX_XXXX` をレジスタ`0x7A`に書き込む。 `XX_XXXX` は影響を与えない。
...
| **\Large ここに書かれていることはアイデアのみで、まったく未テストです。** |
|:-------------------------------------------------------------------------:|
|                                                                           |
